...
$db_sql = "SELECT customer_id,status,first_name,email,eusertype,is_dropshipper FROM `".TABLE_PREFIX."customer` WHERE email = '".$email."' AND registration_type = 'M'";		
...
{
	//check for guest
	$db_sql = "SELECT customer_id,status,first_name,email,eusertype,is_dropshipper FROM `".TABLE_PREFIX."customer` WHERE email = '".$email."' AND registration_type='G' and is_deleted='No'";
	$result = $obj->select($db_sql);
...
{	
	...
	if($registration_type == "Guest"){
		if($email != '')			
		{
			...
			$customer_id = $obj->update(TABLE_PREFIX.'customer', $aData, $where );
			
			$RewardPointVal["customer_id"] = $iCustomerId;
			$RewardPointVal["note"] = "Reward Point Added By Register";
			$RewardPointVal["iRewardpoint"] = 150;
			$RewardDiscountPoint = $obj->insert(TABLE_PREFIX.'reward_point', $RewardPointVal);
		}	
	}
	//convert guest to member
	...
	if($result[0]['status'] == "0"){	//mostly member
		$CustomerArr = array (
			...
			'merge_log' 		=> "Auto updated to Active from social page",
			'status' 			=> '1'
		);
		...
		$cust_upd = $obj->update(TABLE_PREFIX.'customer', $CustomerArr, $where );
	}
