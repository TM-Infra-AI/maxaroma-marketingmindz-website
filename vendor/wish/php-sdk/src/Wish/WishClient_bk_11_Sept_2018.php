...
class WishClient{
  ...
  public function getResponse($type,$path,$params=array()){
	...
	$coderror = $response->getStatusCode();
	$errormsg = '';
    if($coderror==4000){
      $errormsg = "Unauthorized access";
    }
    else if($coderror==1015){
        $errormsg = "Access Token expired";
    }
    else if($coderror==1016){
      $errormsg = "Access Token revoked";
    }
    else if($coderror==1000){
	  $errormsg = "Invalid parameter";
    }
    else if($response->getStatusCode()==1002){
	   $errormsg = "Order has been fulfilled"; 
    }
    else if($coderror!=0){
	  $errormsg = "Unknown error";	
    }
    
    if($coderror!=0 && $errormsg!="Unknown error")
    {
	 ...
	 $mail->Subject = "In Scentworld New Error Generated For Wish API Of Wish Client Class";
	 $mail->MsgHTML('In Scentworld '.$errormsg.' For Wish API on '.date('Y-m-d H:i:s'));
  ...
  public function removeExtraImagesById($id){
   ...
   public function updateShippingById($id,$country,$price,$enabel='false',$wishexpress='false'){
    $params = array('parent_sku'=>$id,'country'=>$country,'price'=>$price,'enabled'=>$enabel,'wish_express' => $wishexpress);
  ...
  public function getShippingById($id,$country){
    $params = array('parent_sku'=>$id,'country'=>$country,'enabled'=>$enabel,'wish_express' => $wishexpress);
