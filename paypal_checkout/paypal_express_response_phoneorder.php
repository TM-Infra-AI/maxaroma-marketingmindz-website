<?php
	...
	$order_id = $_SESSION["phoneorder"]["porder_id"];
	$encorder_id = base64_encode($order_id);
	
	if (isset($_REQUEST['token']) and !empty($_REQUEST['token']))	
	{	
		$token = $_REQUEST['token'];
		$_SESSION["phoneorder"]["token"] = $token;
	}
	## token check end here 
	
	## if token not set then redired on shoppin cart
	if($_SESSION["phoneorder"]["token"] =="" || empty($_SESSION["phoneorder"]["token"]) || $order_id <= 0)
	{
		$ErrorLongMsg = "Error in Processing Request, Please try again.";
		$commonobj->setDisplayMessage($ErrorLongMsg);
		header("location:".$Site_URL."payment/".$encorder_id);
		exit;
	}
	...
	$Data_orders["OrderTotal"] = $OrderRS[0]['order_total'];
	$Data_orders["OrderSubTotal"] = $OrderRS[0]['sub_total'] - $AllDiscounts;
	$Data_orders["ShippingAmt"] = $OrderRS[0]['shipping_amt'];
	$Data_orders["Shipping_Signature"] = $OrderRS[0]['shipping_signature'];
	$Data_orders["TaxAmt"] = $OrderRS[0]['tax'];
	$Data_orders["TotalDiscount"] = $AllDiscounts;
	
	//shipping details
	$Data_orders["shipToName"] = $OrderRS[0]['ship_first_name']." ".$OrderRS[0]['ship_last_name'];
	$Data_orders["shipToStreet"] = $OrderRS[0]['ship_address1'];
	$Data_orders["shipToCity"] = $OrderRS[0]['ship_city'];
	$Data_orders["shipToCountryCode"] = $OrderRS[0]['ship_country'];
	$Data_orders["shipToState"] = $OrderRS[0]['ship_state'];
	$Data_orders["shipToZip"] = $OrderRS[0]['ship_zip'];
	//shipping details
	...
	if(!empty($OrderRS)){
		...
		$tot_count = count($OrderDetailRs);
		for($i=0;$i < $tot_count; $i++){
			$OrderData[$i]["SKU"] = $OrderDetailRs[$i]['sku'];
			$OrderData[$i]["ProductName"] = $OrderDetailRs[$i]['product_name'];
			$OrderData[$i]["Qty"] = $OrderDetailRs[$i]['quantity'];
			$OrderData[$i]["Price"] = $OrderDetailRs[$i]['price'];
			
			if($i == 0){
				$OrderData[$i]["AllDiscounts"] = $AllDiscounts;
			}
		}
		$Data_orders["Order_Details"] = $OrderData;
		
		$finalPaymentAmount =  $OrderRS[0]['order_total'];
		//echo "<pre>"; print_r($Data_orders); exit;
		...
		if($ack == "SUCCESS" || $ack == "SUCCESSWITHWARNING" )	
		{
			 ...
			 * 
				...
				$Data_Shipping_Billing['ship_first_name'] = $first_name;
				$Data_Shipping_Billing['ship_last_name'] = $last_name;
				$Data_Shipping_Billing['ship_company'] 	= '';
				$Data_Shipping_Billing['ship_address1'] = $resArray['SHIPTOSTREET'];
				$Data_Shipping_Billing['ship_address2']= $resArray['SHIPTOSTREET2'];
				$Data_Shipping_Billing['ship_city']= $resArray['SHIPTOCITY'];
				$Data_Shipping_Billing['ship_country']= $resArray['SHIPTOCOUNTRYCODE'];
				$Data_Shipping_Billing['ship_state'] = $resArray['SHIPTOSTATE'];
				$Data_Shipping_Billing['ship_zip']= $resArray['SHIPTOZIP'];
				$Data_Shipping_Billing['ship_phone']= '';
				$Data_Shipping_Billing['ship_email'] = $resArray['EMAIL'];
				
				$Data_Shipping_Billing['bill_first_name'] = $first_name;
				$Data_Shipping_Billing['bill_last_name'] = $last_name;
				$Data_Shipping_Billing['bill_company'] 	= '';
				$Data_Shipping_Billing['bill_address1'] = $resArray['SHIPTOSTREET'];
				$Data_Shipping_Billing['bill_address2'] = $resArray['SHIPTOSTREET2'];
				$Data_Shipping_Billing['bill_city'] = $resArray['SHIPTOCITY'];
				$Data_Shipping_Billing['bill_country']= $resArray['SHIPTOCOUNTRYCODE'];
				$Data_Shipping_Billing['bill_state'] = $resArray['SHIPTOSTATE'];
				$Data_Shipping_Billing['bill_zip'] = $resArray['SHIPTOZIP'];
				$Data_Shipping_Billing['bill_phone']= '';
				$Data_Shipping_Billing['bill_email'] = $resArray['EMAIL'];
				
				$where_cond = " orders_id='".$order_id."' ";			  
				$uporderres = $obj->update(TABLE_PREFIX.'orders',$Data_Shipping_Billing, $where_cond);
				
				$Data_orders["shipToName"] = $Data_Shipping_Billing['ship_first_name']." ".$Data_Shipping_Billing['ship_last_name'];
				$Data_orders["shipToStreet"] = $Data_Shipping_Billing['ship_address1'];
				$Data_orders["shipToCity"] = $Data_Shipping_Billing['ship_city'];
				$Data_orders["shipToCountryCode"] = $Data_Shipping_Billing['ship_country'];
				$Data_orders["shipToState"] = $Data_Shipping_Billing['ship_state'];
				$Data_orders["shipToZip"] = $Data_Shipping_Billing['ship_zip'];
				//update billing and shipping
			*/
			
			$resArray = ConfirmPayment_Phoneorder($Data_orders);
			$ack = strtoupper($resArray["ACK"]);
			//echo "<pre>"; print_r($resArray); exit;
			...
			if($ack=="SUCCESS" or $ack=="SUCCESSWITHWARNING")
			{
				...
				//set email invoices
				$err_msg = "Thank you for you payment. Your order will be processed as soon as possible. An Order Receipt E-mail has been sent to you.";
				$commonobj->setDisplayMessage($err_msg);
				
				$status = base64_encode("1");
				header("location:".$Site_URL."payment/".$encorder_id."/".$status);			
				exit;
			} 
			else  
			{
