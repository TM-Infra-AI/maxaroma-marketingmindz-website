...
for($i=0;$i < $tot_count; $i++){
	...
	if($i == 0){
		...
		$Data_Order[$i]["Other_Charges"] = $OrderRs[$i]['shipping_signature'] + $OrderRs[$i]['gift_charge'];
	}
}
	...
	$udpRefer = $obj->update(TABLE_PREFIX.'orders', $UpdateOrderInformation, " `orders_id` = '".$OrderID."'");
