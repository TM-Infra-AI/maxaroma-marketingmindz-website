...
if($_SESSION["sess_useremail"] == "gequaldev@gmail.com" || $_SERVER['HTTP_X_FORWARDED_FOR'] == "150.129.164.57")
{
	//$PAYPAL_TRANSACTION_MODE = 'sandbox';
}
...
function CallShortcutExpressCheckout($paymentAmount,$returnURL,$cancelURL,$paymentType="Sale",$currencyCodeType="USD",$IsPhonorder="No",$OrderData = array() ) 
{
	############################################################
	...
	$nvpstr = $nvpstr . "&NOSHIPPING=".$NOSHIPPING; ## TO SHOW SHIPPING ADD ON PAYPAL SITE
	$nvpstr = $nvpstr . "&HDRIMG=".$hrdimg;
	$nvpstr = $nvpstr . "&VERSION=53.0";

	$nvpstr = $nvpstr . "&SOLUTIONTYPE=Sole";
	//$nvpstr = $nvpstr . "&USERSELECTEDFUNDINGSOURCE=CreditCard";
	
	$ITEMAMT = $OrderData[0]["SubTotal"] - $OrderData[0]["AllDiscounts"]+ $OrderData[0]["Other_Charges"];
	$SHIPPINGAMT = $OrderData[0]["shipping_amt"];
	$TAXAMT = $OrderData[0]["tax"];
	
	$RouteShippingInsurance = 0;
	if(isset($_SESSION['shipping_insurance']) && $_SESSION['shipping_insurance'] == "Y"){
		$RouteShippingInsurance =  $_SESSION['shipping_insurance_charge'];
	}
	
	$nvpstr = $nvpstr.'&ITEMAMT='.$ITEMAMT.'&SHIPPINGAMT='.$SHIPPINGAMT.'&TAXAMT='.$TAXAMT.'&INSURANCEAMT='.$RouteShippingInsurance;


	##  Shows the Shopping Cart Contents on the Paypal Review Order Page
	if($IsPhonorder == "Yes"){
		$shopping_cart_contents = get_Shopping_Cart_Contents_Phoneorder($OrderData);
	}else{
...
function ConfirmPayment()
{
	...
	$nvpstr .='&ITEMAMT='.$OrderSubTotal.'&SHIPPINGAMT='.$ShippingAmt.'&TAXAMT='.$TaxAmt.'&HANDLINGAMT='.$HandlingAmt.'&INSURANCEAMT='.$RouteShippingInsurance;
...
function ConfirmPayment_Phoneorder($OrderData)
{
	//global $Cart;
	...
	$HandlingAmt = $OrderData["Shipping_Signature"];
	...
	$nvpstr.='&SHIPTONAME='.$OrderData["shipToName"].'&SHIPTOSTREET='.$OrderData["shipToStreet"].'&SHIPTOCITY='.$OrderData["shipToCity"].'&SHIPTOCOUNTRYCODE='.$OrderData["shipToCountryCode"].'&SHIPTOSTATE='.$OrderData["shipToState"].'&SHIPTOZIP='.$OrderData["shipToZip"];
	
	## Shows the Shopping Cart Contents on the Paypal Review Order Page
	$shopping_cart_contents = get_Shopping_Cart_Contents_Phoneorder($OrderData["Order_Details"]);
...
function get_Shopping_Cart_Contents_Phoneorder($OrderData) 
{
	//global $Cart;
	...
	if(count($tempCart) > 0 ) 
	{
		...
		## For the All Type Of Discounts add item with negative amount Start Here
		$TotalDiscount 		= $tempCart[0]["AllDiscounts"];
		$TaxAmount 		= $tempCart[0]["tax"];
		$ShippingAmount	= $tempCart[0]["shipping_amt"];
		$Other_Charges	= $tempCart[0]["Other_Charges"];
		...
		if($Other_Charges > 0)	
		{
			...
			$L_AMT 		= "L_AMT".$tc;
			$L_QTY 		= "L_QTY".$tc;
			$items_description.= "&$L_NUMBER=Other Charges&$L_NAME=Other Charges&$L_QTY=1&$L_AMT=".$Other_Charges."&$L_DESC=".urlencode("Other Charges");
			$tc++;
		}
		...
		if($ShippingAmount > 0)	
		{
			...
			$L_AMT 		= "L_AMT".$tc;
			$L_QTY 		= "L_QTY".$tc;
			$items_description.= "&$L_NUMBER=Shipping Charges&$L_NAME=All Shipping Charges&$L_QTY=1&$L_AMT=".$ShippingAmount."&$L_DESC=".urlencode("All Shipping Charges");
			$tc++;
		}*/
		## For the All Type Of Discounts add item with negative amount End Here
	}
