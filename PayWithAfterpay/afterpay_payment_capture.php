<?php
	...
	if(isset($_REQUEST) && $_REQUEST['return_flag'] == "confirm_payment"){
		...
		$afterpay_transaction_id = $capture_arr['id'];
		...
		if(fopen($myFile, 'a+'))
		{
			...
			$stringData .= chr(13) . chr(13) . 'Capture REQUEST == ' . serialize($payload) . chr(13) . chr(13) ;
			$stringData .= chr(13) . chr(13) . 'Capture RESPONSE == ' . serialize($capture_arr) . chr(13) . chr(13);
		...
		if($capture_arr['status'] == "APPROVED" && ($capture_arr['paymentState'] == "PARTIALLY_CAPTURED" || $capture_arr['paymentState'] == "CAPTURED")){
			...
			$sql = "SELECT payment_gateway_response FROM `".TABLE_PREFIX."orders` WHERE orders_id='".$Cart->getOrderID()."'";
			...
			$updAray = array (
								...
								'afterpay_transaction_id' 	=> $afterpay_transaction_id,
							  );
