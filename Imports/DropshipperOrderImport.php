        ...
        foreach ($rows as $row) 
        {
            if($i > 0) {
                ...
                if($dropshipper_order) {
					...
					$OrderProds = ImportDropshiporder::where('orders_no','=',$temp_dropshipper_order_detail['orders_no'])->get();
						...
						foreach($OrderProds as $OrdProd)
						{
							...
							if($ProductRs && $ProductRs->count() > 0) {
								...
								if($WebsiteStock == 'Out') {
									...
									if(Session::has('aOutOfStockItems')) {
										Session::push('aOutOfStockItems', $OrdProd['sku']);
									} else {
										Session::put('aOutOfStockItems', [$OrdProd['sku']]);
									}
									if(Session::has('aOutOfStockItemsfull')) {
								...
								if($WebsiteStock == 'In') {
									if($ProductRs->current_stock < $OrdProd['quantity']) {
										if(Session::has('HalfaOutOfStockItems') && !in_array($OrdProd['sku'],Session::get('HalfaOutOfStockItems'))) {
												Session::push('HalfaOutOfStockItems', $OrdProd['sku']);
										} else {
											Session::put('HalfaOutOfStockItems', [$OrdProd['sku']]);
										}
										$prd_exit = $ProductRs->current_stock;
									}else{
										$prd_exit = $OrdProd['quantity'];
									}
								}
								...
								if($prd_exit  > 0) {
									...
									if(config('Settings.WHOLESALE_MARKUP') == 'Yes') {
										$specialpricedtl = GetSpecialPricePercentandValue($OrdProd['quantity']);
