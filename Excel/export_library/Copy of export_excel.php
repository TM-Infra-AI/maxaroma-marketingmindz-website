...
if($parent_id ==0)
{
	...
	$Heading =array('Product Code','Product Status','Tibs Own Label','Supplier','Collection','Model','Size','Base Type','Mattress Type','Dimensions when closed','Dimensions when open','Frame Colour','Mattress Colour','With Headboard','Leg Type','Cost of Goods','Cost of Boxes','Cost of Packing',' Credit Card Charge','Cost without Freight','Freight Type','Weight','£ to 20 Kilos','Additional kgs','Rate per Kilo','CPD to Private House',' Total Delivery Cost','Total Cost','Special Case Cost','VAT','retail inc vat','margin in £££s','margin %','suppliers e mail','Auto Order','Number Of Lots','Lead Time','Tibs','Pronto','Description of Collection','Description Of Model','No. of Items','Photo 1','Photo 2','Photo3');
		...
		$Heading =array('Product Code','Product Status','Tibs Own Label','Supplier','Collection','Model','Size','Finish','Colour1','Colour2','Foot End Option','Base Height Adjustment Option','withStorage','Overall Dimension','Headboard Height','Footboard Height','Base Type','Available as Surround','Cover','Cost of Good','Cost of Boxes','Cost of Packing',' Credit Card Charge','Cost without Freight','Freight Type','Weight','£ to 20 Kilos','Additional kgs','Rate per Kilo','CPD to Private House',' Total Delivery Cost','Total Cost','Special Case Cost','VAT','Retail Inc. VAT','Margin in £££s','Margin %','Suppliers Email','Auto Order','Number Of Lots','Lead Time','Tibs','Pronto','Description of Collection','Description Of Model','No. of Items', 'Photo 1','Photo 2','Photo3');
	}
	else if(stristr($cat_name,"head")) //HeadBoards
	{
		$query = "select pcode,product_status,cost_of_packing,special_case_cost,tibsownlabel,
		( SELECT optionvalue FROM tblbed_option WHERE id = supplier ) AS supplier,
		collection, model,
		( SELECT optionvalue FROM tblbed_option WHERE id = size ) AS size,
		( SELECT optionvalue FROM tblbed_option WHERE id = finish ) AS finish,
		 ...
		 ( SELECT optionvalue FROM tblbed_option WHERE id = colour2 ) AS colour2,
		  ...
		  costOfGood,costOfBoxes, creditCardCharge, costWithoutFreight, FreightType, weight,to20Kilos,additionalKgs,ratePKilo,cpdTohouse,totalDeliveryCost, totalCost, vat, retailIncVat, marginInPound, marginPercent,supEmail,autoOrder, numberOfLots,leadTime,pronto,tibs, desCollection,desModel,no_of_items ,".$glob['dbprefix']."img_idx.img1 as photo1,".$glob['dbprefix']."img_idx.img2 as photo2,".$glob['dbprefix']."img_idx.img3 as photo3  from ".$glob['dbprefix']."inventory left join ".$glob['dbprefix']."img_idx on ".$glob['dbprefix']."img_idx.product_id=".$glob['dbprefix']."inventory.product_id where cat_id=".$cat_id;

		$Heading=array('Product Code','Product Status','Tibs Own Label','Supplier',' Collection',' Model',' Size',' Finish','Colour1','Colour2','Overall Dimensions','Floor Standing',' Cover Grade','Cost of Goods','Cost of Boxes','cost_of_packing','Special Case Cost','Credit Card Charge','Cost Without Freight','Freight Type','Weight','£ to 20 kilos',' Additional Kgs','Rate per Kilo','cpd to Private House','Total Delivery Cost','Total Cost','VAT','Retail INC VAT','Margin in £££s',' Margin %','Supplier E-mail','Auto Order','Number of Lots','Lead Time','Tibs','Pronto','Description of Collection','Description of Model','No. of Items' ,'Photo 1','Photo 2','Photo3');
	...
	}else if(stristr($cat_name,"adjust")) //Adjustible Beds
	{
		...
		$Heading=array('Product Code','Product Status','Tibs Own Label','Supplier','Collection','Model','Fillings','Mattress Type','Spring Count','Depth of Visco','Std Remote','Infra Red Remote','Single Massage','Dual Massage','Heavy Duty Moter','Finish','Comfort','Turning Order','Mattress Depth','Base Type','Drawer','Overall Dimensions','Tension','Cover Grade','Size','Cost of Goods',' Cost of Boxes','cost_of_packing','Special Case Cost','Credit Card Charge','Cost Without Freight','Freight Type','Weight','£ to 20 kilos','Additional Kgs','Rate per Kilo','cpd to Private House','Total Delivery Cost','Total Cost','VAT','Retail Inc. VAT','Margin in £££s','Margin %','Supplier E-mail','Auto Order','Number of Lots','Lead Time','Tibs','Pronto','Description of Collection',' Description of Model','No. of Items', 'Photo 1','Photo 2','Photo3');
	}
		...
		collection, model,
		 ...
		 ( SELECT optionvalue FROM tblbed_option WHERE id = dov ) AS dov,
 		 ...
 		 turningOrder,
  		 ( SELECT optionvalue FROM tblbed_option WHERE id = drawer ) AS drawer,
		 mattDepth,
		 costOfGood,costOfBoxes, creditCardCharge, costWithoutFreight, FreightType, weight,to20Kilos,additionalKgs,ratePKilo,cpdTohouse,totalDeliveryCost, totalCost, vat, retailIncVat, marginInPound, marginPercent,supEmail,autoOrder, numberOfLots,leadTime,pronto,tibs, desCollection,desModel,no_of_items,".$glob['dbprefix']."img_idx.img1 as photo1,".$glob['dbprefix']."img_idx.img2 as photo2,".$glob['dbprefix']."img_idx.img3 as photo3  from ".$glob['dbprefix']."inventory left join ".$glob['dbprefix']."img_idx on ".$glob['dbprefix']."img_idx.product_id=".$glob['dbprefix']."inventory.product_id where (".$glob['dbprefix']."inventory.cat_id=".$divcatid. " or ".$glob['dbprefix']."inventory.cat_id=".$matcatid.") ";


		$Heading=array('Product Code','Product Status','Tibs Own Label','Supplier',' Collection',' Model',' Size','Overall Dimensions','Mattress Tick','Base Tick','Base Color','Comfort','Base Type','Tension','Finish','Mattress Type','Fillings','Depth of Visco','Spring Count','Turning Order','No.of Drawers','Mattress Depth','Cost of Goods',' Cost of Boxes','cost_of_packing','Special Case Cost',' Credit Card Charge','Cost Without Freight','Freight Type','Weight',' £ to 20 kilos','Additional Kgs','Rate per Kilo','cpd to Private House','Total Delivery Cost','Total Cost','VAT','Retail Inc. VAT','Margin in £££s','Margin %','Supplier E-mail','Auto Order','Number of Lots','Lead Time','Tibs','Pronto','Description of Collection',' Description of Model','No. of Items','Photo 1','Photo 2','Photo3');
	} else if(stristr($cat_name,"bases")) //Adjustible Beds
	{
		...
		$Heading=array('Product Code','Product Status','Tibs Own Label','Supplier','Model','Size','Base Type','Drawers','Overall Dimensions','Cost of Goods','Cost of Boxes','cost_of_packing','Special Case Cost','Credit Card Charge','Cost Without Freight','Freight Type','Weight','£ to 20 kilos','Additional kgs','Rate per kilo','Cpd to private house','Total delivery cost','Total cost','Vat','Retail inc vat','Margin in £££s','Margin %','Suppliers e mail','Auto order','Number of lots','Lead time','Tibs','Pronto','Description of collection','Description of model','No. of Items', 'Photo 1','Photo 2','Photo3');
	}
	else if(stristr($cat_name,"sofa")) //sofa beds
	{
		$query = "select pcode,product_status,cost_of_packing,special_case_cost,specialdelivery,tibsownlabel,
		( SELECT optionvalue FROM tblbed_option WHERE id = supplier ) AS supplier,
		collection,
		...
		$Heading=array('Product Code','Product Status','Tibs Own Label','Supplier','Collection','Model','Type','Size','Mattress Type','Left Right','Colour1','Colour2','Overall dimensions','Size When Open','Size of Sleeping Surface','Cover grade','Cost of Goods','Cost of Boxes','cost_of_packing','Special Case Cost','Credit Card Charge','Cost Without Freight','Freight Type','Weight','£ to 20 kilos','Additional kgs','Rate per kilo','Cpd to private house','Special Delivery','Total delivery cost','Total cost','Vat','Retail inc vat','Margin in £££s','Margin %','Suppliers e mail','Auto order','Number of lots','Lead time','Tibs','Pronto','Description of collection','Description of model','No. of Items','Photo 1','Photo 2','Photo3');


	}
	else if(stristr($cat_name,"bunks")) //sofa beds
	{
		$query = "select pcode,product_status,cost_of_packing,special_case_cost,specialdelivery,tibsownlabel,
		( SELECT optionvalue FROM tblbed_option WHERE id = supplier ) AS supplier,
		collection,
	 ...
	 ( SELECT optionvalue FROM tblbed_option WHERE id = finish ) AS finish,
		...
		$Heading=array('Product Code','Product Status','Tibs Own Label','Supplier','Collection','Model','Size','Finish','Colour1','Colour2','Overall dimensions','Base Type','Cost of Goods','Cost of Boxes','cost_of_packing','Special Case Cost','Credit Card Charge','Cost Without Freight','Freight Type','Weight','£ to 20 kilos','Additional kgs','Rate per kilo','Cpd to private house','Special Delivery','Total delivery cost','Total cost','Vat','Retail inc vat','Margin in £££s','Margin %','Suppliers e mail','Auto order','Number of lots','Lead time','Tibs','Pronto','Description of collection','Description of model','No. of Items','Photo 1','Photo 2','Photo3');
...
}  else // accessories file code
{
	...
	$Heading=array('Product Code','Product Status','Tibs Own Label','Supplier','Collection','Model','Fillings','Size','Finish','Cover','Colour','Fabric Type','Water Proof','Cost of Goods','Cost of Boxes','cost_of_packing','Special Case Cost','Credit Card Charge','Cost Without Freight','Freight Type','Weight','£ to 20 kilos','Additional kgs','Rate per kilo','Cpd to private house','Total delivery cost','Total cost','Vat','Retail inc vat','Margin in £££s','Margin %','Suppliers e mail','Auto order','Number of lots','Lead time','Tibs','Pronto','Description of collection','Description of model','No. of Items','Photo 1','Photo 2','Photo3');
}
...
if(isset($supplier) and !empty($supplier)) {
		...
		{
		$query .= " and ".$glob['dbprefix']."inventory.supplier in  (select id from tblbed_option where type ='supplier' and optionvalue ='".$supplier."' and (cat_id = $divcatid or cat_id = $matcatid))";
		}else{
...
if($parent_id ==0)
{
	...
	}else if(stristr($cat_name,"frame")) //Bed Frames
	{
		for($i=0; $i<count($results); $i++)
		{
			...
			$tibsownlabel = $results[$i]['tibsownlabel'] == 1?'Y':'N';
			...
			$worksheet2->write($i+1,8,$results[$i]['colour'],$formatot);
			$worksheet2->write($i+1,9,$results[$i]['colour2'],$formatot);
			$worksheet2->write($i+1,10,$results[$i]['footEndOption'],$formatot);
			$worksheet2->write($i+1,11,$baseHeightadjOption,$formatot);
			$worksheet2->write($i+1,12,$results[$i]['withStorage'],$formatot);
			//  $worksheet2->write($i+1,10,$withStorage,$formatot);
			$worksheet2->write($i+1,13,$results[$i]['overallDimension'],$formatot);
			$worksheet2->write($i+1,14,$results[$i]['headboard'],$formatot);
			$worksheet2->write($i+1,15,$results[$i]['footboard'],$formatot);
			$worksheet2->write($i+1,16,$results[$i]['baseType'],$formatot);
			$worksheet2->write($i+1,17,$availableassurround,$formatot);
			$worksheet2->write($i+1,18,$results[$i]['cover'],$formatot);
			$worksheet2->write($i+1,19,$results[$i]['costOfGood'],$formatot);
			$worksheet2->write($i+1,20,$results[$i]['costOfBoxes'],$formatot);
			$worksheet2->write($i+1,21,$results[$i]['cost_of_packing'],$formatot);
			$worksheet2->write($i+1,22,"=round(((AI".($i+2)."/100)*5),2)",$formatot); // Credit card charges
			$worksheet2->write($i+1,23,"=round((T".($i+2)."+U".($i+2)."+W".($i+2)."+V".($i+2)."),2)",$formatot); //Cost Without freight
			$worksheet2->write($i+1,24,$freightType,$formatot);
			$worksheet2->write($i+1,25,$results[$i]['weight'],$formatot);
			$worksheet2->write($i+1,26,$results[$i]['to20Kilos'],$formatot);
			$worksheet2->write($i+1,27,"=if(Z".($i+2)." > 20 , Z".($i+2)."-20,0)",$formatot);  // Aditional kga
			$worksheet2->write($i+1,28,$results[$i]['ratePKilo'],$formatot);
		    $worksheet2->write($i+1,29,$results[$i]['cpdTohouse'],$formatot);
		//	$worksheet2->write($i+1,29,"=round((AA".($i+2)."*AB".($i+2).")+Z".($i+2)."+AC".($i+2).",2)",$formatot);
			$worksheet2->write($i+1,30,"= if(AG".($i+2)." > 0 ,AG".($i+2).",round((AB".($i+2)."*AC".($i+2).")+AA".($i+2)."+AD".($i+2).",2))",$formatot); // Total Delivery Cost
			$worksheet2->write($i+1,31,"=round((X".($i+2)."+AE".($i+2)."),2)",$formatot); // Total Cost
			...
			$worksheet2->write($i+1,41,$tibs,$formatot);
			$worksheet2->write($i+1,42,$pronto,$formatot);
	...
	}else if(stristr($cat_name,"head")) //head boards
	{
		for($i=0; $i<count($results); $i++)
		{
			...
			$worksheet2->write($i+1,8,$results[$i]['colour'],$formatot);
			$worksheet2->write($i+1,9,$results[$i]['colour2'],$formatot);
			$worksheet2->write($i+1,10,$results[$i]['overallDimension'],$formatot);
			$worksheet2->write($i+1,11,$floorStanding ,$formatot);
			$worksheet2->write($i+1,12,$results[$i]['cover'],$formatot);
			$worksheet2->write($i+1,13,$results[$i]['costOfGood'],$formatot);
			$worksheet2->write($i+1,14,$results[$i]['costOfBoxes'],$formatot);
			$worksheet2->write($i+1,15,$results[$i]['cost_of_packing'],$formatot);
			$worksheet2->write($i+1,16,$results[$i]['special_case_cost'],$formatot);
			$worksheet2->write($i+1,17,"=round(((AC".($i+2)."/100)*5),2)",$formatot); // Credit card charges
			$worksheet2->write($i+1,18,"=round((N".($i+2)."+O".($i+2)."+P".($i+2)."+R".($i+2)."),2)",$formatot); // cost without freight
			...
			$worksheet2->write($i+1,25,"= if(Q".($i+2)." > 0 ,Q".($i+2).",round((W".($i+2)."*X".($i+2).")+V".($i+2)."+Y".($i+2).",2))",$formatot); // Total Delivery Cost
			$worksheet2->write($i+1,26,"=round((S".($i+2)."+Z".($i+2)."),2)",$formatot); // total cost
			...
			$worksheet2->write($i+1,35,$tibs,$formatot);
			$worksheet2->write($i+1,36,$pronto,$formatot);
	...
	}else if(stristr($cat_name,"adjust")) //Adjustable
	{
		for($i=0; $i<count($results); $i++)
		{
			...
			$worksheet2->write($i+1,4,$results[$i]['collection'],$formatot);
			$worksheet2->write($i+1,5,$results[$i]['model'],$formatot);
			$worksheet2->write($i+1,6,$results[$i]['fillings'],$formatot);
			$worksheet2->write($i+1,7,$results[$i]['mattressType'],$formatot);
			$worksheet2->write($i+1,8,$results[$i]['springCount'],$formatot);
			$worksheet2->write($i+1,9,$results[$i]['dov'],$formatot);
			$worksheet2->write($i+1,10,$std_remote,$formatot);
			$worksheet2->write($i+1,11,$infra_remote,$formatot);
			$worksheet2->write($i+1,12,$single_massage_unit,$formatot);
			$worksheet2->write($i+1,13,$dual_massage_unit,$formatot);
			$worksheet2->write($i+1,14,$heavy_duty_motor,$formatot);
			$worksheet2->write($i+1,15,$results[$i]['finish'],$formatot);
			$worksheet2->write($i+1,16,$results[$i]['comfort'],$formatot);
			$worksheet2->write($i+1,17,$turningOrder,$formatot);
			$worksheet2->write($i+1,18,$results[$i]['mattDepth'],$formatot);
			$worksheet2->write($i+1,19,$results[$i]['baseType'],$formatot);
			$worksheet2->write($i+1,20,$results[$i]['drawer'],$formatot);
			$worksheet2->write($i+1,21,$results[$i]['overallDimension'],$formatot);
			$worksheet2->write($i+1,22,$results[$i]['tension'],$formatot);
			$worksheet2->write($i+1,23,$results[$i]['cover'],$formatot);
			$worksheet2->write($i+1,24,$results[$i]['size'],$formatot);
			$worksheet2->write($i+1,25,$results[$i]['costOfGood'],$formatot);
			$worksheet2->write($i+1,26,$results[$i]['costOfBoxes'],$formatot);
			$worksheet2->write($i+1,27,$results[$i]['cost_of_packing'],$formatot);
			$worksheet2->write($i+1,28,$results[$i]['special_case_cost'],$formatot);
			$worksheet2->write($i+1,29,"=round(((AO".($i+2)."/100)*5),2)",$formatot);
			$worksheet2->write($i+1,30,"=round((Z".($i+2)."+AA".($i+2)."+AB".($i+2)."+AD".($i+2)."),2)",$formatot);
			$worksheet2->write($i+1,31,$freightType,$formatot);
			$worksheet2->write($i+1,32,$results[$i]['weight'],$formatot);
			$worksheet2->write($i+1,33,$results[$i]['to20Kilos'],$formatot);
//			$worksheet2->write($i+1,31,$results[$i]['additionalKgs'],$formatot);
			$worksheet2->write($i+1,34,"=if(AG".($i+2)." > 20 , AG".($i+2)."-20,0)",$formatot);
			$worksheet2->write($i+1,35,$results[$i]['ratePKilo'],$formatot);
			$worksheet2->write($i+1,36,$results[$i]['cpdTohouse'],$formatot);
...
//			$worksheet2->write($i+1,36,"=round((AH".($i+2)."*AI".($i+2).")+AG".($i+2)."+AJ".($i+2).",2)",$formatot);
			$worksheet2->write($i+1,37,"= if(AC".($i+2)." > 0 ,AC".($i+2).",round((AI".($i+2)."*AJ".($i+2).")+AH".($i+2)."+AK".($i+2).",2))",$formatot); // Total Delivery Cost
			$worksheet2->write($i+1,38,"=round((AE".($i+2)."+AL".($i+2)."),2)",$formatot); //total Cost
			...
			$worksheet2->write($i+1,47,$tibs,$formatot);
			$worksheet2->write($i+1,48,$pronto,$formatot);
...
else if((stristr($cat_name,"divan") and stristr($cat_name,"bed")) || (stristr($cat_name,"mattr") and !stristr($cat_name,"top") and !stristr($cat_name,"prot"))) //Divans
	{
		for($i=0; $i<count($results); $i++)
		{
			...
			$worksheet2->write($i+1,8,$results[$i]['mattressTick'],$formatot);
			$worksheet2->write($i+1,9,$results[$i]['baseTick'],$formatot);
			$worksheet2->write($i+1,10,$results[$i]['baseColor'],$formatot);
			$worksheet2->write($i+1,11,$results[$i]['comfort'],$formatot);
			$worksheet2->write($i+1,12,$results[$i]['baseType'],$formatot);
			$worksheet2->write($i+1,13,$results[$i]['tension'],$formatot);
			$worksheet2->write($i+1,14,$results[$i]['finish'],$formatot);
			$worksheet2->write($i+1,15,$results[$i]['mattressType'],$formatot);
			$worksheet2->write($i+1,16,$results[$i]['fillings'],$formatot);
			$worksheet2->write($i+1,17,$results[$i]['dov'],$formatot);
			$worksheet2->write($i+1,18,$results[$i]['springCount'],$formatot);
			$worksheet2->write($i+1,19,$turningOrder,$formatot);
			$worksheet2->write($i+1,20,$results[$i]['drawer'],$formatot);
			$worksheet2->write($i+1,21,$results[$i]['mattDepth'],$formatot);
			$worksheet2->write($i+1,22,$results[$i]['costOfGood'],$formatot);
			$worksheet2->write($i+1,23,$results[$i]['costOfBoxes'],$formatot);
			$worksheet2->write($i+1,24,$results[$i]['cost_of_packing'],$formatot);
			$worksheet2->write($i+1,25,$results[$i]['special_case_cost'],$formatot);
			$worksheet2->write($i+1,26,"=round(((AL".($i+2)."/100)*5),2)",$formatot);
			$worksheet2->write($i+1,27,"=round((W".($i+2)."+X".($i+2)."+Y".($i+2)."+AA".($i+2)."),2)",$formatot);
			$worksheet2->write($i+1,28,$freightType,$formatot);
			$worksheet2->write($i+1,29,$results[$i]['weight'],$formatot);
			$worksheet2->write($i+1,30,$results[$i]['to20Kilos'],$formatot);
			$worksheet2->write($i+1,31,"=if(AD".($i+2)." > 20 , AD".($i+2)."-20,0)",$formatot);
			$worksheet2->write($i+1,32,$results[$i]['ratePKilo'],$formatot);
			$worksheet2->write($i+1,33,$results[$i]['cpdTohouse'],$formatot);
//			$worksheet2->write($i+1,33,"=round((AE".($i+2)."*AF".($i+2).")+AD".($i+2)."+AG".($i+2).",2)",$formatot);
			$worksheet2->write($i+1,34,"= if(Z".($i+2)." > 0 ,Z".($i+2).",round((AF".($i+2)."*AG".($i+2).")+AE".($i+2)."+AH".($i+2).",2))",$formatot); // Total Delivery Cost

			//$worksheet2->write($i+1,31,$results[$i]['totalDeliveryCost'],$formatot); //AE
			$worksheet2->write($i+1,35,"=round((AB".($i+2)."+AI".($i+2)."),2)",$formatot); // Total cost
			...
			$worksheet2->write($i+1,44,$tibs,$formatot);
			$worksheet2->write($i+1,45,$pronto,$formatot);
	...
	}else if(stristr($cat_name,"bases")) //Bed Frames
	{
		for($i=0; $i<count($results); $i++)
		{
			...
			$worksheet2->write($i+1,7,$results[$i]['drawer'],$formatot);
			$worksheet2->write($i+1,8,$results[$i]['overallDimension'],$formatot);
			$worksheet2->write($i+1,9,$results[$i]['costOfGood'],$formatot);
			$worksheet2->write($i+1,10,$results[$i]['costOfBoxes'],$formatot);
			$worksheet2->write($i+1,11,$results[$i]['cost_of_packing'],$formatot);
			$worksheet2->write($i+1,12,$results[$i]['special_case_cost'],$formatot);
			$worksheet2->write($i+1,13,"=round(((Y".($i+2)."/100)*5),2)",$formatot);
			$worksheet2->write($i+1,14,"=round((J".($i+2)."+K".($i+2)."+L".($i+2)."+N".($i+2)."),2)",$formatot);
			$worksheet2->write($i+1,15,$freightType,$formatot);
			$worksheet2->write($i+1,16,$results[$i]['weight'],$formatot);
			$worksheet2->write($i+1,17,$results[$i]['to20Kilos'],$formatot);
//			$worksheet2->write($i+1,15,$results[$i]['additionalKgs'],$formatot);
			$worksheet2->write($i+1,18,"=if(Q".($i+2)." > 20 , Q".($i+2)."-20,0)",$formatot);
			$worksheet2->write($i+1,19,$results[$i]['ratePKilo'],$formatot);
			$worksheet2->write($i+1,20,$results[$i]['cpdTohouse'],$formatot);
...
//			$worksheet2->write($i+1,20,"=round((R".($i+2)."*S".($i+2).")+Q".($i+2)."+T".($i+2).",2)",$formatot);
			$worksheet2->write($i+1,21,"= if(M".($i+2)." > 0 ,M".($i+2).",round((S".($i+2)."*T".($i+2).")+R".($i+2)."+U".($i+2).",2))",$formatot); // Total Delivery Cost
			$worksheet2->write($i+1,22,"=round((V".($i+2)."+O".($i+2)."),2)",$formatot); // Total cost
			...
			$worksheet2->write($i+1,31,$tibs,$formatot);
			$worksheet2->write($i+1,32,$pronto,$formatot);
	...
	else if(stristr($cat_name,"sofa")) //Sofa
	{
		...
		$autoOrder = $results[$i]['autoOrder'] == 1?'Yes':'No';
			...
			$worksheet2->write($i+1,4,$results[$i]['collection'],$formatot);
			$worksheet2->write($i+1,5,$results[$i]['model'],$formatot);
			$worksheet2->write($i+1,6,$results[$i]['type'],$formatot);
			$worksheet2->write($i+1,7,$results[$i]['size'],$formatot);
			$worksheet2->write($i+1,8,$results[$i]['mattressType'],$formatot);
			$worksheet2->write($i+1,9,$results[$i]['leftright'],$formatot);
			$worksheet2->write($i+1,10,$results[$i]['colour'],$formatot);
			$worksheet2->write($i+1,11,isset($results[$i]['colour2'])?$results[$i]['colour2']:'',$formatot);
			$worksheet2->write($i+1,12,$results[$i]['overallDimension'],$formatot);
			$worksheet2->write($i+1,13,$results[$i]['sizeOpen'],$formatot);
			$worksheet2->write($i+1,14,$results[$i]['sizeSleepingSurface'],$formatot);
			$worksheet2->write($i+1,15,$results[$i]['cover'],$formatot);
			$worksheet2->write($i+1,16,$results[$i]['costOfGood'],$formatot);
			$worksheet2->write($i+1,17,$results[$i]['costOfBoxes'],$formatot);
			$worksheet2->write($i+1,18,$results[$i]['cost_of_packing'],$formatot);
			$worksheet2->write($i+1,19,$results[$i]['special_case_cost'],$formatot);
			$worksheet2->write($i+1,20,"=round(((AG".($i+2)."/100)*5),2)",$formatot);
			$worksheet2->write($i+1,21,"=round((Q".($i+2)."+R".($i+2)."+S".($i+2)."+U".($i+2)."),2)",$formatot);
			$worksheet2->write($i+1,22,$freightType,$formatot);
			$worksheet2->write($i+1,23,$results[$i]['weight'],$formatot);
			$worksheet2->write($i+1,24,$results[$i]['to20Kilos'],$formatot);
//			$worksheet2->write($i+1,22,$results[$i]['additionalKgs'],$formatot);
			$worksheet2->write($i+1,25,"=if(X".($i+2)." > 20 , X".($i+2)."-20,0)",$formatot);
			$worksheet2->write($i+1,26,$results[$i]['ratePKilo'],$formatot);
			$worksheet2->write($i+1,27,$results[$i]['cpdTohouse'],$formatot);
			$worksheet2->write($i+1,28,$specialDelivery,$formatot);
...
//			$worksheet2->write($i+1,27,"=round((Y".($i+2)."*Z".($i+2).")+X".($i+2)."+AA".($i+2).",2)",$formatot);
			$worksheet2->write($i+1,29,"= if(T".($i+2)." > 0 ,T".($i+2).",round((Z".($i+2)."*AA".($i+2).")+Y".($i+2)."+AB".($i+2).",2))",$formatot); // Total Delivery Cost
			$worksheet2->write($i+1,30,"=round((V".($i+2)."+AD".($i+2)."),2)",$formatot); // Total cost
			$worksheet2->write($i+1,31,"=round(AG".($i+2)."-(AG".($i+2)."/1.175),2)",$formatot); // vat
			$worksheet2->write($i+1,32,$results[$i]['retailIncVat'],$formatot);
			$worksheet2->write($i+1,33,"=round((AG".($i+2)."/1.175)-AE".($i+2).",2)",$formatot); // margin in pounds
			$worksheet2->write($i+1,34,"=round((AH".($i+2)."/(AG".($i+2)."/1.175)*100),2)",$formatot); // margin in percent
			$worksheet2->write($i+1,35,$results[$i]['supEmail'],$formatot);
			$worksheet2->write($i+1,36,$autoOrder,$formatot);
			$worksheet2->write($i+1,37,$results[$i]['numberOfLots'],$formatot);
			$worksheet2->write($i+1,38,$results[$i]['leadTime'],$formatot);
			$worksheet2->write($i+1,39,$tibs,$formatot);
			$worksheet2->write($i+1,40,$pronto,$formatot);
			$worksheet2->write($i+1,41,$results[$i]['desCollection'],$formatot);
			$worksheet2->write($i+1,42,$results[$i]['desModel'],$formatot);
			$worksheet2->write($i+1,43,$results[$i]['no_of_items'],$formatot);
			$worksheet2->write($i+1,44,$results[$i]['photo1'],$formatot);
			$worksheet2->write($i+1,45,$results[$i]['photo2'],$formatot);
			$worksheet2->write($i+1,46,$results[$i]['photo3'],$formatot);
		} //end for
	}
	else if(stristr($cat_name,"bunks")) //Sofa
	{
		for($i=0; $i<count($results); $i++)
		{
		...
		$autoOrder = $results[$i]['autoOrder'] == 1?'Yes':'No';
			...
			$worksheet2->write($i+1,7,$results[$i]['finish'],$formatot);
			$worksheet2->write($i+1,8,$results[$i]['colour'],$formatot);
			$worksheet2->write($i+1,9,isset($results[$i]['colour2'])?$results[$i]['colour2']:'',$formatot);
			$worksheet2->write($i+1,10,$results[$i]['overallDimension'],$formatot);
...
} else //Export fields for all Accessories Categories
{
	...
	for($i=0; $i<count($results); $i++)
	{
		...
		$worksheet2->write($i+1,6,$results[$i]['fillings'],$formatot);
		$worksheet2->write($i+1,7,$results[$i]['size'],$formatot);
		$worksheet2->write($i+1,8,$results[$i]['finish'],$formatot);
		$worksheet2->write($i+1,9,$results[$i]['cover'],$formatot);
		$worksheet2->write($i+1,10,$results[$i]['colour'],$formatot);
		$worksheet2->write($i+1,11,$results[$i]['fabricType'],$formatot);
		$worksheet2->write($i+1,12,$waterproof,$formatot);
		$worksheet2->write($i+1,13,$results[$i]['costOfGood'],$formatot);
		$worksheet2->write($i+1,14,$results[$i]['costOfBoxes'],$formatot);
		$worksheet2->write($i+1,15,$results[$i]['cost_of_packing'],$formatot);
		$worksheet2->write($i+1,16,$results[$i]['special_case_cost'],$formatot);
		$worksheet2->write($i+1,17,"=round(((AC".($i+2)."/100)*5),2)",$formatot);
		$worksheet2->write($i+1,18,"=round((N".($i+2)."+O".($i+2)."+P".($i+2)."+R".($i+2)."),2)",$formatot);
		$worksheet2->write($i+1,19,$freightType,$formatot);
		$worksheet2->write($i+1,20,$results[$i]['weight'],$formatot);
		$worksheet2->write($i+1,21,$results[$i]['to20Kilos'],$formatot);
		$worksheet2->write($i+1,22,"=if(U".($i+2)." > 20 , U".($i+2)."-20,0)",$formatot);
		$worksheet2->write($i+1,23,$results[$i]['ratePKilo'],$formatot);
		$worksheet2->write($i+1,24,$results[$i]['cpdTohouse'],$formatot);
		$worksheet2->write($i+1,25,"= if(Q".($i+2)." > 0 ,Q".($i+2).",round((W".($i+2)."*X".($i+2).")+V".($i+2)."+Y".($i+2).",2))",$formatot); // Total Delivery Cost
		$worksheet2->write($i+1,26,"=round((S".($i+2)."+Z".($i+2)."+Q".($i+2)."),2)",$formatot); // total cost
		$worksheet2->write($i+1,27,"=round(AC".($i+2)."-(AC".($i+2)."/1.175),2)",$formatot); // Vat
		$worksheet2->write($i+1,28,$results[$i]['retailIncVat'],$formatot);
		$worksheet2->write($i+1,29,"=round((AC".($i+2)."/1.175)-AA".($i+2).",2)",$formatot); // Margin in pounds
		$worksheet2->write($i+1,30,"=round((AD".($i+2)."/(AC".($i+2)."/1.175)*100),2)",$formatot); // Margin in percent
