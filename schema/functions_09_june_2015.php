...
function getProductDetailSchema()
{
	...
	$ProductDetailSchema .= '"description": "'.$productDetail[0]['product_description'].'",
			  "name": "'.strip_tags($productDetail[0]['product_name']).'",
			  "offers": {
				"@type": "Offer",
...
function getSchema($schema_type)
{
	...
	{		
		...
		foreach($SliderManufacture as $Manufacturer)
		{
			$Manufacturerlist .= '{
			  "@type": "Brand",
			  "image": "'.$Manufacturer['manufacture_logo'].'",
			  "url": "'.$Manufacturer['manufacture_link'].'",
			  "name": "'.$Manufacturer['manufacture_name'].'"
			}';
		...
		$schema .= '"@context": "http://schema.org",
		  "@type": "ItemList",
		 "numberOfItems": "'.$totmanu.'",
		  "itemListElement": ['.$Manufacturerlist.']';
		//Brand List End
		...
		$schema .= '"@context": "http://schema.org",
		  ...
		  "itemListElement": '.$prodiuctlistschema.'';
...
function getProductListSchema($type)
{
	if($type=='productlist')
	{
		...
		$productlist .= '"@context": "http://schema.org",
		  "@type": "ItemList",
		  "url": "'.$CanonicalURL.'",
		  "numberOfItems": "'.$totpro.'",
		  "itemListElement": '.$productlist2.'';
		...
		else
		{
			...
			if($totnewarrival>0)
			{
				...
				$productlist .= '"@context": "http://schema.org",
				  "@type": "ItemList",
				  "name": "New Arrival",
				  "numberOfItems": "'.$totnewarrival.'",
				  "itemListElement": '.$newarrivallist.'';
			}
			...
			if($tottopseller>0)
			{
				...
				$productlist .= '"@context": "http://schema.org",
				  "@type": "ItemList",
				  "name": "New Arrival",
				  "numberOfItems": "'.$tottopseller.'",
				  "itemListElement": '.$topsellerlist.'';
			}
			...
			if($totclearance>0)
			{
				...
				$productlist .= '"@context": "http://schema.org",
				  "@type": "ItemList",
				  "name": "Clearance",
				  "numberOfItems": "'.$totclearance.'",
				  "itemListElement": '.$clearancelist.'';
			}
		}
		...
		if($totreference>0)
		{
			...
			for($i=0; $i<$totreference; $i++)
			{
				...
				$productlist .= '"@context": "http://schema.org",
				  "@type": "ItemList",
				  "name": "'.$tmpname.'",
				  "numberOfItems": "'.$totrefpro.'",
				  "itemListElement": '.$refprolist.'';
			}		
		}
