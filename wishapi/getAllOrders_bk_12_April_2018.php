						...
						$OrderID = $obj->insert(TABLE_PREFIX.'wish_orders', $OrderInsert);
						 ...
						 if(isset($OrderID) && !empty($OrderID))
						 {
						    ...
						    { 
								...
								$sql = "SELECT p.*, ".$EXTRA_SQL.",".$casewhen.",".$StockCondition.",".$WebsiteCurrentStock." FROM `".TABLE_PREFIX."products` as p WHERE p.status = '1' AND p.sku = '".trim($OrderDetailRes[$j]['sku'])."'";
						
								$ProductRs =  $obj->select($sql);
								
								if($ProductRs[0]["WebsiteStock"]=="Out")
								{
									if($ProductRs[0]["cosmo_sku"]!='' &&  $ProductRs[0]["cosmo_current_stock"] > 0 &&  $ProductRs[0]["cosmo_our_price"] > 0)
									{
										$IsCosmo = "Yes";
										$VendorSKU = $ProductRs[0]["cosmo_sku"];
									}
									else if($ProductRs[0]["nandansons_sku"]!='' &&  $ProductRs[0]["nandansons_current_stock"] > 0 && $ProductRs[0]["nandansons_our_price"] > 0)
									{
										$IsNandansons = "Yes";
										$VendorSKU = $ProductRs[0]["nandansons_sku"];
									}
									else if($ProductRs[0]["perfumeworldwide_sku"]!='' &&  $ProductRs[0]["perfumeworldwide_currentstock"] > 0 && $ProductRs[0]["perfumeworldwide_our_price"] > 0)
									{
										$IsPerfumePW  = "Yes";
										$VendorSKU = $ProductRs[0]["perfumeworldwide_sku"];
									}
								}
									if(($IsCosmo=="Yes" || $IsNandansons=='Yes' || $IsPerfumePW=='Yes' ) && $VendorSKU!='')
									{
										$IsVender = "Yes";
									}
