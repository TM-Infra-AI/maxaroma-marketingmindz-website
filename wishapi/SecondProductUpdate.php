...
$myFile = '/home/maxaroma/public_html/Logs/SecondProductUpdate.txt';
if(filesize($myFile)==10000000)
{
	rename($myFile,"/home/maxaroma/public_html/Logs/SecondProductUpdate_bk.txt");
	unlink($myFile);
}
...
if(fopen($myFile, 'a+'))
{
	...
	$stringData .= date("m/d/Y H:i:s")." : SecondProductUpdate.php :  Wish Second Product Update  : Wish Second Product Update Start\n";
...
$newsql .= " FROM `" . TABLE_PREFIX . "products`,`" . TABLE_PREFIX . "products_one`, `" . TABLE_PREFIX . "products_category` ,`" . TABLE_PREFIX . "category`, `" . TABLE_PREFIX . "manufacture` as m,`".TABLE_PREFIX."brand`" . $extraTables;
$newsql .= " WHERE 1 ";
...
$newsql .="  AND `" . TABLE_PREFIX . "products_one`.is_wish_created='Yes' ";
$newsql .="  AND `".TABLE_PREFIX."products`.brand_id = `".TABLE_PREFIX."brand`.brand_id "; 
$newsql .="  AND `".TABLE_PREFIX."products`.products_id = `".TABLE_PREFIX."products_one`.products_id "; 
...
if(fopen($myFile, 'a+'))
{
	$fh = fopen($myFile, 'a+');
	$stringData = "";
	$stringData .= date("m/d/Y H:i:s")." : SecondProductUpdate.php :  Wish Second Product Update  : Wish Second Product Update End\n";
	fwrite($fh, $stringData);
	fclose($fh);
}
