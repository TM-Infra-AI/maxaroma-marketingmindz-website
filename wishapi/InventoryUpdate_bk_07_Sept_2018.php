...
if($total_prod > 0)
{
	for($b=0; $b < $total_record_batch; $b++ )
	{
		...
		for($i=0;$i<$TotalProducts;$i++)
		{
			...
			$var = $client->updateInventoryBySKU($sku,$prodRes[$i]["current_stock"]);
		...
		try {
			...
			$sendmessage = "Wish Product Inventory Failed In Wish API On Maxaroma At ".date('m-d-Y') . " And The Product SKU Is ".$prodRes[$i]["sku"];	
			$test 	= @mail($vemail,"Wish Product Inventory Failed In Wish API On Maxaroma",$sendmessage);
...
$vemail = "naresh.qualdev@gmail.com";
$sendmessage = "Wish Product Inventory Cron In Wish API On Maxaroma At ".date('m-d-Y');	
$test 	= @mail($vemail,"Wish Product Inventory Cron In Wish API On Maxaroma",$sendmessage);
