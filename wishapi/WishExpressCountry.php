...
$myFile = '/home/maxaroma/public_html/Logs/WishExpressCountry.txt';
if(filesize($myFile)==10000000)
{
	rename($myFile,"/home/maxaroma/public_html/Logs/WishExpressCountry_bk.txt");
	unlink($myFile);
}
...
if(fopen($myFile, 'a+'))
{
	...
	$stringData .= date("m/d/Y H:i:s")." : WishExpressCountry.php :  Wish Express Country : Wish Express Country Start\n";
...
$sql .="  AND `" . TABLE_PREFIX . "products_one`.is_wish_express='No' ";
...
if($total_prod > 0)
{
	...
	{
		 $sel_pro = " SELECT DISTINCT(`" . TABLE_PREFIX . "products`.sku),`" . TABLE_PREFIX . "category`.parent_id, `" . TABLE_PREFIX . "products`.current_stock, `" . TABLE_PREFIX . "products`.products_id, ";
		 $sel_pro .= $fetch;
		 ...
		 $sel_pro .="  AND `" . TABLE_PREFIX . "products_one`.is_wish_express='No' ";
	  ...
	  //  echo $sel_pro; exit;
		...
		for($i=0;$i<$TotalProducts;$i++)
		{
			...
			$sku						= $prodRes[$i]['sku'];
			$product_id					= $prodRes[$i]['products_id'];			
			...
			else
			{
			 ...
			 $udpRefer = $obj->update(TABLE_PREFIX.'products_one', $wishArr, " `products_id` = '".$product_id."'");
...
if(fopen($myFile, 'a+'))
{
	$fh = fopen($myFile, 'a+');
	$stringData = "";
	$stringData .= date("m/d/Y H:i:s")." : WishExpressCountry.php :  Wish Express Country : Wish Express Country End\n";
	fwrite($fh, $stringData);
	fclose($fh);
}
